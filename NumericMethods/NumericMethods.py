...
def run_kut_4(x, h, first_y):
    ...
    for i in range(1, np.size(x)):
        y = y_list[i-1] + h / 6 * (real_function(x[i-1], y_list[i-1])
...
def addams_4(x, h, first_y):
    y_list = [first_y]
    y = y_list[0] + h * real_function(x[0], y_list[0])
    y_list.append(y)
    y = y_list[1] + h * real_function(x[1], y_list[1])
    y_list.append(y)
    y = y_list[2] + h * real_function(x[2], y_list[2])
    y_list.append(y)
    for i in range(4, np.size(x)):
        y = y_list[i-1] + h/24*(55*real_function(x[i-1], y_list[i-1])-59*real_function(x[i-2], y_list[i-2])
...
def addams_3(x, h, first_y):
    ...
    for i in range(3, np.size(x)):
        y = y_list[i-1] + h/12*(23*real_function(x[i-1], y_list[i-1]) - 16*real_function(x[i-2], y_list[i-2])
...
def euler_method(x, h, first_y):
    y_list = [first_y]
    for i in range(1, np.size(x)):
        y = y_list[i-1] + h * real_function(x[i], y_list[i-1])
        y_list.append(y)
    return y_list
...
def euler_modificated(x, h, first_y):
    y_list = [first_y]
    for i in range(1, np.size(x)):
        y = y_list[i-1] + h * real_function(x[i-1]+h/2, y_list[i-1] + h/2 * real_function(x[i-1], y_list[i-1]))
        y_list.append(y)
    return y_list
...
def euler_recount(x, h, first_y):
    y_list = [first_y]
    for i in range(1, np.size(x)):
        y = y_list[i-1] + h / 2 * (real_function(x[i-1], y_list[i-1])
...
def euler_central(x, h, first_y):
    ...
    for i in range(2, np.size(x)):
        y = y_list[i-2] + 2 * h * real_function(x[i-1], y_list[i-1])
        y_list.append(y)
    return y_list
